...
class CustomDatasetFromImages(Dataset):
    def __init__(self, myimage, mylabel, rootDir, transform=None):
        
        d = {'dir': [myimage], 'label': [mylabel]}
        
        self.annotations = pd.DataFrame(data=d)

        #self.annox = pd.DataFrame()
        

        #self.annox = pd.DataFrame(data=d)
